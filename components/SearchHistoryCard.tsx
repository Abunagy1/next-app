...
import { Plane, Building2, Clock, Search, Building, Users, Type, ChevronsLeft } from 'lucide-react';
    ...
    <Card className="w-full max-w-md rounded-2xl border-primary shadow-md transition-shadow hover:shadow-xl">
      <CardContent className="flex items-center gap-2 p-3">
        ...
        <div className="flex flex-1 grow flex-col">
          <p className="text-base font-semibold leading-snug text-foreground">{search.details}</p>
          {search.type === 'hotel' && (
            <div className="mt-2 space-y-1 text-xs text-muted-foreground">
              ...
              <div className="flex items-center">
                ...
                <NoSSR fallback={<Skeleton className="h-3 w-[100px]" />}>
                  <ShowTimeInClientSide date={search.checkInDate!} formatStr="MMM d, yyyy" />
                </NoSSR>
              </div>
              <div className="flex items-center">
                <Clock className="mr-1 h-3 w-3" />
                Check-out:&nbsp;
                <NoSSR fallback={<Skeleton className="h-3 w-[100px]" />}>
                  <ShowTimeInClientSide date={search.checkOutDate!} formatStr="MMM d, yyyy" />
                </NoSSR>
              </div>
            </div>
          )}
          ...
          <div className="mt-2 flex items-center text-xs text-muted-foreground">
            ...
            Searched on:&nbsp;
            <NoSSR fallback={<Skeleton className="h-3 w-[100px]" />}>
              <ShowTimeInClientSide date={search.searchedAt} formatStr="MMM d, yyyy h:mm a" />
            </NoSSR>
          </div>
        </div>
        <Link
