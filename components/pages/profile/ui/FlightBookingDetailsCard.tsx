...
import { capitalize, cn, minutesToHMFormat } from '@/app/lib/utils';
...
import { DeleteFlightBookingButton } from '@/components/pages/flights.[flightId]/sections/DeleteFlightBookingButton';
interface Passenger {
  key: string;
  fullName: string;
...
interface FlightBookingDetailsCardProps {
  className?: string;
  bookingData: BookingData;
}

export function FlightBookingDetailsCard({
  className,
  bookingData,
}: FlightBookingDetailsCardProps) {
  const {
    key,
    bookingStatus,
    paymentStatus,
    ...
    cancellationPolicy = {},   // ← default empty object
    pnrCode,
    passengers,
    segments,
  } = bookingData;

  const statusBookingBgColor =
  ...
  const { canCancel, canRefund, canDownload, canDelete, canPay, canConfirm } = allowedFlightBookingActionBtns(
    bookingStatus,
    paymentStatus,
    cancellationPolicy,
    departureDate,
    ...
    <div className={cn('space-y-6 rounded-2xl border bg-white p-5 shadow-sm dark:bg-gray-800 dark:border-gray-700', className)}>
      ...
      <div className="grid grid-cols-1 items-start gap-4 md:grid-cols-[1fr_auto]">
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium dark:text-white">Flight No: {itineraryFlightNumber}</p>
          <p className="text-sm font-medium dark:text-white">PNR: {pnrCode}</p>
        </div>
        ...
        <div key={s.key} className="space-y-4 rounded-xl border p-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex min-w-[200px] items-center gap-4">
              <Image
                src={airlinesLogos[s.airlineIataCode as keyof typeof airlinesLogos] || airlinesLogos.EK}
                height={64}
                width={64}
              ...
              />
              <div>
                <p className="text-base font-semibold dark:text-white">{s.airlineName}</p>
            ...
            <div className="text-right">
              <p className="text-base font-semibold dark:text-white">{s.flightNumber}</p>
              <p className="text-sm text-muted-foreground dark:text-gray-400">{s.airplaneModelName}</p>
            </div>
          </div>

          {/* Timeline */}
          <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
            <div className="text-center text-sm font-medium md:text-left dark:text-white">
              <NoSSR fallback="hh:mm aaa">
                <ShowTimeInClientSide date={s.departureDateTime} formatStr="hh:mm aaa" />
              </NoSSR>
              <br />
              <NoSSR fallback="dd MMM yyyy">
                <ShowTimeInClientSide date={s.departureDateTime} formatStr="d MMM yyyy" />
              </NoSSR>
            ...
            <div className="text-center text-sm font-medium md:text-right">
              <NoSSR fallback="hh:mm aaa">
                <ShowTimeInClientSide date={s.arrivalDateTime} formatStr="hh:mm aaa" />
              </NoSSR>
              <br />
              <NoSSR fallback="dd MMM yyyy">
                <ShowTimeInClientSide date={s.arrivalDateTime} formatStr="d MMM yyyy" />
              </NoSSR>
              <div className="text-xs text-muted-foreground">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            />
            <SmallDataCard imgSrc={airplaneIcon} title="Terminal" data={s.terminal || '—'} />
          </div>
        </div>
      ))}

      {/* Passengers */}
      <div className="rounded-xl bg-muted/50 p-4 dark:bg-gray-700/50">
         <h3 className="mb-2 text-sm font-semibold text-muted-foreground dark:text-gray-300">Passengers</h3>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {passengers.map((p) => (
            <div key={p.key || p.fullName} className="rounded-xl bg-white p-4 shadow-sm dark:bg-gray-800">
              <p className="font-medium dark:text-white">{p.fullName}</p>
              <p className="text-sm text-muted-foreground dark:text-gray-400">Type: {p.passengerType}</p>
              <p className="text-sm text-muted-foreground dark:text-gray-400">Class: {p.seatClass}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-wrap justify-end gap-3 border-t pt-4 dark:border-gray-700">
        ...
        {canDownload && (
          <Button variant="outline" className="min-w-[100px]" asChild>
            <Link href={`/user/my_bookings/flights/${key}/ticket`}>Download Ticket(s)</Link>
          </Button>
        )}
        {canCancel && <CancelFlightBtn pnrCode={pnrCode} className="" />}
        {canRefund && <RequestRefundFlightBtn pnrCode={pnrCode} className="" />}
        {canDelete && <DeleteFlightBookingButton bookingId={key} />}
      </div>
    </div>
  );
}