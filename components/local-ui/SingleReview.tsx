...
import flagReviewAction from '@/app/lib/actions/flagReviewAction';
import { usePathname } from 'next/navigation';
import { useToast } from '@/components/ui/use-toast';
import avatarDefault from '@/public/travel/icons/avatar-default.svg';
...
export function SingleReview({ review, session }: SingleReviewProps) {
  ...
  async function handleFlaged() {
    if (!session?.user) {
      toast({ description: 'You have to login first', variant: 'destructive' });
      return;
    }
    setIsFlagging(true);
    const result = await flagReviewAction(pathname, review._id, session.user.id);
    setIsFlagging(false);
    if (typeof result === 'string') {
      toast({
        description: result === 'ADDED' ? 'Review flagged' : 'Flag removed',
        variant: 'default',
      });
  ...
  return isLoading ? (
    ...
    <>
      <div className="flex items-start gap-[16px]">
        <Avatar className="h-[45px] w-[45px] rounded-full">
          <AvatarImage
        ...
        <div className="w-full text-[0.875rem]">
          <div className="mb-[8px] flex items-center gap-1">
            <span className="font-semibold">
              ...
              {RATING_SCALE[Math.floor(review.rating) as keyof typeof RATING_SCALE]}
            </span>
            <span>|</span>
            <span>{user.name}</span>
          </div>
        ...
        <div className="flex items-center gap-1">
          <span className="text-xs font-bold text-gray-400">
            {review.flagged.length === 0 ? '' : review.flagged.length}
          </span>
          <Button className="h-auto p-1 hover:bg-slate-200" variant="ghost" onClick={handleFlaged}>
            {isFlagging ? (
              ...
              >
                <path
                  ...
                  fill={
                    review.flagged.includes(session?.user?.id || '')
                      ? 'hsl(6,93%,71%)'
                      : '#112211'
                  }
                />
