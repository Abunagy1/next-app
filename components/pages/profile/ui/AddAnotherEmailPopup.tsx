...
import { addNewEmailAction } from '@/app/lib/actions/addNewEmailAction';
...
interface AddAnotherEmailPopupState {
  success?: boolean;
  message?: string;
...
export function AddAnotherEmailPopup() {
  const [state, dispatch] = useActionState<AddAnotherEmailPopupState, FormData>(addNewEmailAction, {});
  ...
  useEffect(() => {
    if (state?.success === true) {
      ...
      toast.success(state.message || 'Email added successfully');
    }
  }, [state]);
  ...
  return (
    <Dialog open={opened} onOpenChange={setOpened}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          className="gap-1 p-2 h-auto text-foreground dark:text-white"
        >
          <svg
            width="20"
            height="20"
      ...
      <DialogContent className="sm:max-w-[425px] dark:bg-gray-800 dark:text-white">
        <DialogHeader>
          <DialogTitle className="dark:text-white">Add new email</DialogTitle>
        ...
        <form id="add-email-form" action={dispatch}>
          <Input
            label="Email"
            type="email"
            name="email"
