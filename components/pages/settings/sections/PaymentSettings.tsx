...
import deletePaymentCardAction from '@/app/lib/actions/deletePaymentCardAction';
...
export default function PaymentSettings() {
  ...
  async function handlePaymentCardDeleteClick(e: React.MouseEvent<HTMLButtonElement>, cardId: string) {
    ...
    setOpen(false);
    e.currentTarget.disabled = false; // re-enable handled by tryAgain
  }
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          {!loading && !fetchError.status && cardData.length === 0 && (
            <EmptyResult className="w-full" message="No cards found." description="" />
          )}
          {!loading && !fetchError.status && cardData.length > 0 && (
            <div>
              {cardData.map((card) => (
                <div key={card.id} className="mb-2 flex items-center justify-between">
                  <p className="flex items-center gap-2 font-medium">
                    <Image
                      src={`/travel/icons/cards/${card.cardType}.svg`}
                      height={70}
                      width={70}
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                          ...
                          onClick={(e) => handlePaymentCardDeleteClick(e, card.id)}
                        >
                          Continue
                        </AlertDialogAction>
                      </AlertDialogFooter>
          ...
          <AddPaymentCard
            ...
            customAddButtonElement={
              <Button className="mt-4 text-secondary dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700" size="sm" variant="outline">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <Button variant="outline" className="dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700">Download PDF</Button>
        </CardContent>
      </Card>
    </div>
  );
