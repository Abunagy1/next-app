...
import { deletePaymentCardAction } from '@/app/lib/actions/updateProfileActions';
...
export function PaymentCard({ card, tryAgain = () => {} }: PaymentCardProps) {
  ...
  async function handleClick() {
    ...
    if (res?.success === false) {
      toast.error(res?.message || 'Failed to delete payment card');
    } else {
  ...
  return (
    <div className={cn('flex h-[212px] min-h-[200px] w-[378px] min-w-[300px] flex-col justify-between rounded-[16px] bg-primary p-[16px] dark:bg-gray-700', deleting && 'grayscale')}>
      <div className="flex items-start justify-between">
        <div className="font-semibold leading-none">
          <p className="text-[1.5rem] dark:text-white">**** **** ****</p>
          <p className="text-[2rem] dark:text-white">{card.last4Digits}</p>
        </div>
        <AlertDialog open={open} onOpenChange={setOpen}>
          <AlertDialogTrigger asChild title="Delete Payment Method">
            <Button variant="ghost" size="icon" className="h-auto w-auto p-1 hover:text-destructive">
              <svg
                width="22"
                height="20"
                viewBox="0 0 22 20"
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[0.75rem] font-medium dark:text-gray-300">Valid Thru</p>
          <p className="text-[1.25rem] font-semibold dark:text-white">{card.validTill}</p>
        </div>
        <Image
