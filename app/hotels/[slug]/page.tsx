...
import location from "@/public/travel/icons/location.svg";
import { getManyDocs } from "@/app/lib/db/getOperationDB"; // getOneDoc
...
import { HotelDetails } from "@/components/pages/hotels.[slug]/sections/HotelDetails";
import {
  Dialog,
  DialogClose,
  DialogContent,
...
import { ShareHotelButton } from '@/components/pages/hotels.[slug]/sections/ShareHotelButton';
...
export default async function HotelDetailsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const hotelDetails = await getHotel(slug, searchState) as HotelData | null;
  if (!hotelDetails || Object.keys(hotelDetails).length === 0) {
    return notFound();
  }
  // Ensure _id is defined for PostgreSQL
  (hotelDetails as any)._id = (hotelDetails as any)._id || (hotelDetails as any).id;
  const hotelId = dbType === 'postgres' ? (hotelDetails as any).id : hotelDetails._id;
  let reviews: any[] = [];
  if (dbType === 'postgres') {
    reviews = await sql`
      SELECT * FROM hotel_reviews
      WHERE hotel_id = ${hotelId} AND slug = ${slug}
    `;
  } else {
    reviews = await getManyDocs(
      "HotelReview",
      { hotelId, slug },
      [hotelDetails._id + "_review", slug + "_review", "hotelReviews"]
    );
  }
  ...
  const price = cheapestRoom ? hotelPriceCalculation(cheapestRoom.price, 1) : { total: 0, totalBeforeDiscount: 0, discountPercentage: 0 };
  ...
  if (session?.user) {
    ...
    isLiked = userDetails?.hotels?.bookmarked?.includes(hotelDetails._id) || false;
  }
  const groupByRoomType = groupBy(roomsSorted, (room: any) => room.roomType);
  return (
    <main className="mx-auto mb-[90px] mt-10 w-[90%]">
      ...
      <div className="my-[40px] flex flex-col items-center justify-between gap-5 sm:flex-row">
        <div>
          <div className="mb-[16px] flex items-center gap-[16px]">
            <h1 className="text-[1.5rem] font-bold text-secondary">{hotelDetails.name}</h1>
          </div>
          ...
          <div className="flex items-center gap-1 text-[0.75rem]">
            ...
            <p className="font-bold">{totalReviewsCount ? ratingScale : "N/A"}</p>
        ...
        <div>
          ...
          <div className="flex flex-col gap-[16px] sm:flex-row">
            ...
            <ShareHotelButton />
            <Button asChild>
              <Link href={`/hotels/${slug}/book`}>Book now</Link>
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="relative mb-[40px] grid w-auto grid-cols-4 grid-rows-2 gap-[8px] overflow-hidden rounded-[12px] max-lg:aspect-video lg:h-[500px]">
        ...
        {hotelDetails.images.length > 1 &&
          hotelDetails.images.slice(1, 5).map((image: string) => (
            <Image
              key={image}
              height={1000}
        ...
        <Dialog>
          <DialogTrigger asChild>
            <Button size="icon" className="absolute bottom-[8px] right-[8px]">
              <View />
            </Button>
          </DialogTrigger>
          <DialogContent close={false} className="h-[90vh] max-w-[90%] items-center border-0 bg-transparent">
            <DialogClose asChild className="absolute right-[16px] top-[16px] rounded-md border-2 border-white text-white">
              <X />
            </DialogClose>
            <Carousel opts={{ autoPlay: false }} className="mx-auto max-w-[80%] sm:max-w-[90%]">
              <CarouselContent>
                {hotelDetails.images.length > 0 &&
                  hotelDetails.images.map((img: string, i: number) => (
                    <CarouselItem key={img} className="flex h-full w-full items-center justify-center">
                      <Image
                        key={img}
                        src={img}
                        alt={`Hotel image ${i + 1}`}
      ...
      <div>
        ...
        <p className="mb-[32px] font-medium opacity-75">{hotelDetails.description}</p>
        <div className="gobye-scrollbar flex gap-[16px] overflow-x-auto pb-3">
      ...
      <div>
        ...
        <div className="space-y-6">
          {Object.entries(groupByRoomType).map(([roomType, rooms]: any) => (
            <Dropdown key={roomType} defaultOpen={true} title={roomType}>
              <div className="space-y-4 p-4">
                {rooms.slice(0, 3).map((room: any) => (
                  <div key={room._id} className="group flex items-center justify-between rounded-md border-b p-1 pb-4 hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-800 dark:text-white">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-medium group-hover:underline dark:text-white">
                          {room.bedOptions}
                        </p>
                        <p className="text-xs font-bold opacity-60 dark:text-gray-300">
                          {formatCurrency(hotelPriceCalculation(room.price, 1).total)} / night
                        </p>
                        <p className="text-xs opacity-60 dark:text-gray-400">
                          Person capacity: {room.sleepsCount}
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <div>
          <ul className="grid grid-cols-2 gap-[24px]">
            {hotelDetails.amenities.map((amenity: string) => (
              <li key={amenity} className="flex items-center gap-[8px] font-semibold">
                {amenity}
              </li>
            ))}
          </ul>
