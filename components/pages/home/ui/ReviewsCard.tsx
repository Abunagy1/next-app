'use client';
import { generateStars } from '@/app/lib/utils.client';
...
export function ReviewsCard({ comment, rate, reviewer, profileImage }: ReviewsCardProps) {
        ...
        <div className="space-y-6">
          ...
          <div className="mt-auto">
            ...
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-500">({rate?.toFixed(1)})</span>
            </div>
            {/* Reviewer info */}
            <div className="border-t border-gray-100 pt-4">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-primary/20 to-primary/10">
                  {profileImage ? (
                    <Image src={profileImage} alt="profile_image" className="h-full w-full rounded-full" width={40} height={40} />
                  ) : (
                    <span className="text-lg font-bold text-primary">{reviewer.charAt(0).toUpperCase()}</span>
                  )}
                </div>
                <div>
                  <p className="text-sm font-semibold text-gray-900">{reviewer}</p>
                  <p className="text-xs font-medium text-gray-500">Verified Customer</p>
                </div>
              </div>
            </div>
          </div>
