...
const PreferencesSection = ({ nextStep }: any) => {
  ...
  useLayoutEffect(() => {
    ...
    success = parsePDetails.every((el: any) => {
  ...
  return shouldRender ? (
    <>
      <div className="mx-auto flex w-full flex-col gap-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <Button onClick={() => setProgress(nextStep)}>Continue</Button>
        </div>
        {passengersFormVals.map((passenger: any, index: number) => (
          <PreferencesForm key={index} passenger={passenger} index={index} />
        ))}
        <Button onClick={() => setProgress(nextStep)}>Continue</Button>
      </div>
    </>
    ...
    <div className="flex h-full min-h-[300px] flex-col items-center justify-center gap-4 rounded-md border border-red-300 bg-red-50 p-6 shadow-sm">
      ...
      <p className="max-w-md text-center text-sm text-red-700">
        Please complete the passenger details form before proceeding to seat preferences.
      </p>
      <Button
        size="lg"
