...
import { getWebsiteReviews, getWebsiteReviewsStats } from '@/app/lib/services';
...
export async function Reviews() {
  ...
  if (userId) {
    if (dbType === 'postgres') {
      const rows = await sql`
        SELECT * FROM website_reviews WHERE user_id = ${userId} LIMIT 1
      `;
      if (rows.length) userReview = rows[0];
    } else {
      await connectDB();
      userReview = await getOneDoc(
        'WebsiteReview',
    ...
    hasAlreadyReviewed = Object.keys(userReview).length > 0;
  }
  ...
  return (
    <section className="relative mx-auto mb-[80px] overflow-hidden px-4 dark:bg-transparent">
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mb-16">
          <SectionTitle
            ...
            subTitle="Discover what our valued customers say about their GoBye experience"
          />
          <div className="mx-auto mt-12 grid max-w-4xl grid-cols-2 gap-6 md:grid-cols-4">
            {/* Stats cards – unchanged, they use the passed values */}
          ...
          <div className="mb-12">
            <div className="mb-8 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Badge
                  variant="outline"
                  className="rounded-full border-primary/30 bg-primary/10 px-4 py-2 font-semibold dark:bg-primary/20 dark:border-primary/50 dark:text-white">
                  Customer Reviews
                </Badge>
