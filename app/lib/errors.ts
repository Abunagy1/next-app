...
export const ErrorCodes: Record<string, ErrorCodeDefinition> = {
  //SYS
...
export class AppError extends Error {
  ...
  retryable: boolean;
  guidance: string;
  constructor(errorObj: ErrorCodeDefinition, statusCode?: number) {
    super(errorObj.message);
    this.name = errorObj.name;
    this.statusCode = statusCode ?? errorObj.statusCode;
    this.errorCode = errorObj.code;
    this.category = errorObj.category;
    this.userMessage = errorObj.userMessage;
    this.retryable = errorObj.retryable;
    this.guidance = errorObj.guidance;
  }
}
/**
...
//   return err;
// }
export function createAppError(errorObj: ErrorCodeDefinition, statusCode?: number): AppError {
  return new AppError(errorObj, statusCode);
}
/**
 ...
 */
export function mapDatabaseError(dbError: DatabaseError): ErrorCodeDefinition {
 ...
 * @returns {Object} Mapped error object
 */
export function mapHttpError(statusCode: number, message = ""): ErrorCodeDefinition {
  switch (statusCode) {
    case 400: return ErrorCodes.INVALID_FORMAT;
    case 401: return ErrorCodes.TOKEN_EXPIRED;
    case 403: return ErrorCodes.ACCESS_DENIED;
    case 404: return ErrorCodes.FLIGHT_NOT_FOUND;
    case 409: return ErrorCodes.FLIGHT_FULLY_BOOKED;
    case 422: return ErrorCodes.MISSING_REQUIRED_FIELDS;
    ...
    case 504: return ErrorCodes.EXTERNAL_API_TIMEOUT;
 ...
 */
export function createErrorResponse(errorObj: ErrorCodeDefinition, options = {}): Response {
 ...
 */
export function handleError(error: AppError | DatabaseError | Error, options = {}): StructuredErrorResponse {
